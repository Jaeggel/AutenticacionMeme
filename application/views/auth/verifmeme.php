	...
	<div class="row main">
		...
		<div class="panel-heading">
	       <div class="panel-title text-center">
	       		...
	       		<hr/>
	       		<h3 class="title">C.I.<?php echo $this->session->userdata("cedula")?></h3>
	       		<hr/>
	       	</div>
	    </div>
    </div>
...
<div class="main-center">
	<div class="form-group">
		<div>
			<button type="submit" id="authen" class="btn btn-primary btn-lg btn-block login-button">Comprobar Meme</button></br>
		</div>
	</div>
</div>
...
var datos;
 $('#authen').on('click',function(){
	...
	var meme1=id;
	var emoji1=ide;
	var top1 = $('#top-text').val().toLocaleUpperCase();
	var bottom1 = $('#bottom-text').val().toLocaleUpperCase();
    ...
    function cambiar(datos){
        if (datos) {
            alert('Su meme es válido');
            window.location="verificar2";
        }else{
            alert('Su Meme No Es El Mismo.');
